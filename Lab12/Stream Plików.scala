...
dbutils.fs.mkdirs("/FileStore/tables/data/AWS")

for (i <- 0 to 39) {
    dbutils.fs.cp(s"dbfs:/databricks-datasets/structured-streaming/events/file-$i.json", s"/FileStore/tables/data/AWS/file-$i.json")
}
...
// Databricks notebook source
val inputPath = "dbfs:/FileStore/tables/data/AWS/"
...
// val schemaSource = scala.io.Source.fromFile(url.getFile).getLines.mkString
// val schemaFromJson = DataType.fromJson(schemaSource).asInstanceOf[StructType]
// val staticInputDF = spark.createDataFrame(
...
static_counts.createOrReplaceTempView("staticCountsDF")
