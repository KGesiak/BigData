...
df.write.format("delta").partitionBy("Country").mode("overwrite").save("dbfs:/FileStore/tables/dataSaved")
val delta = spark.read
  .format("delta").load("dbfs:/FileStore/tables/dataSaved").repartition(500)

delta.rdd.getNumPartitions
...
spark.sql(s"""
  CREATE TABLE table
  USING Delta
  LOCATION 'dbfs:/FileStore/tables/dataSaved'
""")
...
val sqlZorderQuery = timeIt(spark.sql("select * from table where StockCode = '22301'").collect())

// COMMAND ----------

// MAGIC %sql
// MAGIC select * from table where StockCode = '22301'
...
// TODO
val poZorderQuery = timeIt(spark.sql("select * from table where StockCode = '22301'").collect())
...
// TODO
val plikiPrzed = dbutils.fs.ls("dbfs:/FileStore/tables/dataSaved/Country=Sweden").length
...
// TODO
val plikiPo = dbutils.fs.ls("dbfs:/FileStore/tables/dataSaved/Country=Sweden").length
